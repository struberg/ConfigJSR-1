 ...
 *
 * <h3>Implicit Converter methods</h3>
 *
 * <p>If no explicit Converter and no built-in Converter could be found for a certain type,
 * the {@code Config} invokes an <em>Implicit Converter method</em>, if (priority in the given order)</p>
 * <ul>
 *     <li>The target type {@code T} has a Constructor with a single parameter type that is assignable to {@link CharSequence}, or</li>
 *     <li>the target type {@code T} has a method {@code static T valueOf(CharSequence)}, with a parameter type that is assignable to {@link CharSequence}, or</li>
 *     <li>the target type {@code T} has a method {@code static T parse(CharSequence)}, with a parameter type that is assignable to {@link CharSequence}, or</li>
 *     <li>the target type {@code T} is an {@code Enum}, in which case the {@link Enum#valueOf(Class, String)} is being used.</li>
 * </ul>
 *
 * If the class of the target type defines several, overloaded <em>Implicit Converter methods</em> that would match the target signature,
 * for example both {@code static T valueOf(CharSequence)} and {@code static T valueOf(String)}, the implementation will decide which
 * method is selected (and non-portable behavior results).
 *
